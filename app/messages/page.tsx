...
export default function MessagesPage() {
  ...
  async function loadEmails() {
    ...
    // fetch last 100 emails (no date filter) to get the user's email
    const r = await fetch("/api/nylas/messages");
    if (r.status === 401) { router.push("/signup"); return; }

    const data = await r.json();
    if (data.error) { router.push("/signup"); return; }

    const email: string = data.email || "";

    // check if this user already has a sheet tab
    const idsRes = await fetch(`/api/sheets/existing-ids?email=${encodeURIComponent(email)}`);
    const idsData = await idsRes.json();
    const isNewUser = (idsData.ids || []).length === 0;

    let msgs: any[];

    if (isNewUser) {
      // new user — use all 100 emails already fetched
      msgs = (data.messages || []).map(normalise).filter((m: any) => {
        const text = `${m.subject} ${m.from} ${m.body}`.toLowerCase();
        return text.includes("airbnb") || text.includes("aircierge");
      });
    } else {
      // existing user — re-fetch only today's emails
      const todayMidnight = new Date();
      todayMidnight.setHours(0, 0, 0, 0);
      const r2 = await fetch(`/api/nylas/messages?since=${encodeURIComponent(todayMidnight.toISOString())}`);
      const data2 = await r2.json();
      msgs = (data2.messages || []).map(normalise).filter((m: any) => {
        const text = `${m.subject} ${m.from} ${m.body}`.toLowerCase();
        return text.includes("airbnb") || text.includes("aircierge");
      });
    }
    ...
    handleSync(msgs, email);
  }

  useEffect(() => { loadEmails(); }, []);

  async function handleSync(autoMsgs?: any[], autoEmail?: string) {
    const email = autoEmail ?? emailRef.current;
    const todayEmails = autoMsgs ?? msgsRef.current;
    if (!email || !todayEmails.length) { setSyncing(false); return; }
    ...
    try {
      // get thread IDs already in the sheet — dedup before calling webhook
      const idsRes = await fetch(`/api/sheets/existing-ids?email=${encodeURIComponent(email)}`);
      const idsData = await idsRes.json();
      const existingThreadIds = new Set<string>((idsData.ids || []).map((id: string) => id.toLowerCase()));

      const newEmails = todayEmails.filter((m: any) => {
        const tid = (m.threadId || m.id || "").toLowerCase();
        return !existingThreadIds.has(tid);
      });

      if (newEmails.length === 0) {
        setNoNewEmails(new Date().toISOString().split("T")[0]);
        setSyncing(false);
        return;
      }

      // ensure sheet exists
      const ensureData = await fetch("/api/sheets/ensure", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email }),
      }).then(r => r.json());
      setSheetExists(!!(ensureData.exists || ensureData.created));

      const batches: any[][] = [];
      for (let i = 0; i < newEmails.length; i += CHUNK_SIZE) batches.push(newEmails.slice(i, i + CHUNK_SIZE));
      ...
      for (const batch of batches) {
        ...
        await fetch("/api/sheets/write", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email, rows: batchRows }),
        });
      }

      setLastSyncedDate(new Date().toISOString().split("T")[0]);
    } catch (err) {
        ...
        {syncing && (
          ...
          >
            ...
            <span style={{ fontSize: 13, color: "#22c55e" }}>
              Syncing emails — fetching latest reservations…
            </span>
            ...
            <div style={statCard}>
              <div
                style={{
                  fontSize: 14,
                  fontWeight: 600,
                  color: syncing ? "#f59e0b" : lastSyncedDate ? "#22c55e" : "#666",
                }}
              >
                {syncing ? "Syncing…" : lastSyncedDate ? `Last synced: ${new Date(lastSyncedDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}` : "Never synced"}
              </div>
              <div style={statLabel}>Last sync</div>
            </div>
            {sheetExists !== null && (
              <div style={statCard}>
                <div
              ...
              >
                ...
                >
                  <span style={{ color: "#555" }}>
                    {uploadedRows.length} rows total
                  </span>
                  {uploadStats && (
                    <>
                      <span style={{ color: "#22c55e", fontWeight: 600 }}>
                        {uploadStats.unique} unique
                      </span>
                      <span style={{ color: "#f59e0b", fontWeight: 600 }}>
                        {uploadStats.dupes} duplicates (will be skipped)
                      </span>
                    </>
                  )}
                </div>
                ...
                onClick={async () => {
                  ...
                  if (email && data.length > 0) {
                    const idsRes = await fetch(
                      `/api/sheets/existing-ids?email=${encodeURIComponent(
                        email
                      )}`
                    );
                    const idsData = await idsRes.json();
