...
export async function GET() {
  try {
    ...
    if (rows.length < 2) {
      ...
      return Response.json({ success: true, count: 0, range: "last 90 days + future", rows: [] });
    }
    ...
    return Response.json({ success: true, count: filtered.length, range: "last 90 days + future", rows: filtered });
  } catch (error) {
